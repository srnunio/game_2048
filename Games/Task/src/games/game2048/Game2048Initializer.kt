...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val randomCell = generateCell(board)
        val randomValue = generateRandomStartValue()
        var pair: Pair<Cell, Int>? = null
        if (randomCell != null) pair = Pair(randomCell, randomValue)
        return pair

    }

    private fun generateCell(board: GameBoard<Int?>): Cell? {
        val emptyCells = board.getAllCells().filter { board[it] == null }

        if (emptyCells.isEmpty()) return null

        return emptyCells.shuffled(java.util.Random()).first()
    }
}